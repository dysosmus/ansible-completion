...
_ansible_complete_host() {
    ...
    local grep_opts="-o"
    inventory_file=$(_ansible_get_inventory_file)
    ...
    local hosts
    hosts=$(ansible ${inventory_file:+-i "$inventory_file"} all --list-hosts 2>&1 \
        && [[ -e "$inventory_file" ]] \
        && [[ -d "$inventory_file" ]] || [[ ! -x "$inventory_file" ]] \
...
_ansible_get_module_list() {
    local module_path
    local hash_module_path
    module_path=$(_ansible_get_module_path)
    hash_module_path=$(_md5 "$module_path")
    ...
    if [[ -f "$cache_file" ]]; then
        local timestamp=$(($(_timestamp) - $(_timestamp_last_modified "$cache_file")))
...
_ansible_complete_option_module_name() {
    local current_word=$1
    local module_list
    module_list=$(_ansible_get_module_list)
